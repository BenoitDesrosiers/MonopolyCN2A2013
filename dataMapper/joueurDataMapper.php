...
class JoueurDataMapper extends Mapper {
    ...
    public static function updateArgent($argentMontant, $joueur, $nouveauMontant) {
    	$queryTxt = 'update joueurpartie_argent set ArgentMontant=?,
				 JoueurPartieUsagerCompte=?, JoueurPartiePartieEnCoursId=?,
				Quantite=? where JoueurPartieUsagerCompte=? and ArgentMontant=?';
    	$query = self::$db->prepare($queryTxt);
    	
    	$values= array ($argentMontant,
    			$joueur->getCompte(),
    			1,
    			$nouveauMontant[$argentMontant],
    			$joueur->getCompte(),
    			$argentMontant);
